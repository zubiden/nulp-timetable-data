...
async function doParsing() {
	getInstitutes().then(inst => {
		writeFile(path.join(exportPath, "institutes.json"), JSON.stringify(inst, null, 4));
	}).catch(err => {
...
async function fetchTimetables(groups) {
	...
	for(let group of groups) {
		await getTimetable(group).then(timetable => {
...
async function getGroups(institute) {
	...
	let groups = await parser.getGroups()
